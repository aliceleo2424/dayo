...
-- Adds an admin-only 1,000 KRW PortOne E2E product without changing the trial product or index.
-- The server API must also require payment_test=true and verify profiles.id/auth UID role=admin.
...
begin
  ...
  case p_product_key
    ...
    when 'admin_test_1000' then
      v_product_name := '결제 테스트 1,000원';
      v_amount := 1000;
      v_ticket_count := 1;
    else
  ...
  if p_product_key = 'admin_test_1000' and not exists (
    select 1
